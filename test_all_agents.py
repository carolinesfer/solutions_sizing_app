...
# Import agents and schemas using full module paths
# Note: We don't add custom_model directories to sys.path to avoid namespace collisions
# (agents use absolute imports like "from config import Config" which would conflict)
...
async def test_questionnaire_agent(
    facts: FactExtractionModel,
) -> QuestionnaireDraft | None:
    ...
    try:
        ...
        print_result(
            "✅ Output",
            {
                ...
                "selected_from_master_ids": result.selected_from_master_ids[:5]
                if result.selected_from_master_ids
                else [],
    ...
    try:
        ...
        print_result(
            "✅ Output",
            {
                ...
                "gaps": result.gaps,
                "sample_qas": [
                    {"id": qa["id"], "answer": qa["answer"]} for qa in result.qas[:3]
                ],
            },
        )
...
async def main() -> None:
    ...
    print(
        f"   USE_DATAROBOT_LLM_GATEWAY: {os.getenv('USE_DATAROBOT_LLM_GATEWAY', 'not set')}"
    )
    print(
        f"   DATAROBOT_API_TOKEN: {'set' if os.getenv('DATAROBOT_API_TOKEN') else 'not set'}"
    )
    ...
    if os.getenv("INFRA_ENABLE_LLM") == "gateway_direct.py":
        if not os.getenv("DATAROBOT_API_TOKEN") or not os.getenv("DATAROBOT_ENDPOINT"):
            ...
            print(
                "   Please set DATAROBOT_API_TOKEN and DATAROBOT_ENDPOINT environment variables."
            )
            print(
                "   Or set INFRA_ENABLE_LLM to a different value to use direct OpenAI."
            )
            print("\n   Example:")

...
[Modeling](../index.html "Modeling") > [Specialized workflows](index.html
"Specialized workflows") > Bias and Fairness resources

# Bias and Fairness resources¶

The tools of the Bias and Fairness feature test your models for bias. This
allows you identify bias before (or after) models are deployed and then to
take action before the model's decisions cause negative outcomes for your
organization. See a more complete overview and terminology
[here](../../reference/pred-ai-ref/bias-ref.html).
...
The tools available for each step of working with Bias and Fairness are
described in the following sections. Fairness metrics and terminology are
described in the [Bias and Fairness reference](../../reference/pred-ai-
ref/bias-ref.html).

Topic | Description  
---|---  
**Settings**  
[Advanced options: fairness metrics](../build-models/adv-opt/fairness-metrics.html#set-fairness-metrics) | Set fairness metrics prior to model building (or [from the Leaderboard](../build-models/adv-opt/fairness-metrics.html#configure-metrics-and-mitigation-post-autopilot) post-modeling).  
[Advanced options: mitigation](../build-models/adv-opt/fairness-metrics.html#set-mitigation-techniques) | Set mitigation techniques prior to model building (or [from the Leaderboard](../build-models/adv-opt/fairness-metrics.html#configure-metrics-and-mitigation-post-autopilot) post-modeling).  
**Model insights**  
[Per-Class Bias](../analyze-models/bias/per-class.html) | Identify if a model is biased, and if so, how much and who it's biased towards or against.  
[Cross-Class Data Disparity](../analyze-models/bias/cross-data.html) | Depict why a model is biased, and where in the training data it learned that bias from.  
[Cross-Class Accuracy](../analyze-models/bias/cross-acc.html) | Measure the model's accuracy for each class segment of the protected feature.  
[Bias vs Accuracy](../analyze-models/other/bias-tab.html) | View the tradeoff between predictive accuracy and fairness.  
**Deployments**  
[Fairness monitoring](../../mlops/deployment-settings/fairness-settings.html) | Configure tests that allow models to recognize, in real-time, when protected features in the dataset fail to meet predefined fairness conditions.  
[Per-Class Bias](../../mlops/governance/mlops-fairness.html#view-per-class-bias) | Uses the fairness threshold and score of each class to determine if certain classes are experiencing bias in the model's predictive behavior.  
[Fairness over time](../../mlops/governance/mlops-fairness.html#view-fairness-over-time) | View how the distribution of a protected feature's fairness scores have changed over time.  
**Reference**  
[Bias and Fairness reference](../../reference/pred-ai-ref/bias-ref.html) | View a brief overview and see the methods used to calculate fairness and to identify biases in the model's predictive behavior.  

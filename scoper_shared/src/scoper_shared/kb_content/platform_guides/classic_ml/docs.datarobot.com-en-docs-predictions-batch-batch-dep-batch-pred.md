[ DataRobot docs ](../../../index.html "DataRobot docs")
  ...
  * [ Predictions ](../../index.html) [ Predictions ](../../index.html)
    * [ Real-time scoring methods ](../../realtime/index.html) [ Real-time scoring methods ](../../realtime/index.html)
      * [ Prediction API snippets ](../../realtime/code-py.html)
      * [ Qlik predictions ](../../realtime/integration-code-snippets.html)
    * [ Batch prediction methods ](../index.html) [ Batch prediction methods ](../index.html)
      * [ Batch prediction UI ](index.html) [ Batch prediction UI ](index.html)
        * [ Make a one-time batch prediction ](batch-pred.html) [ Make a one-time batch prediction ](batch-pred.html)
...
Make a one-time batch prediction
        ...
        * [ Schedule recurring batch prediction jobs ](batch-pred-jobs.html)
        * [ Manage prediction job definitions ](manage-pred-job-def.html)
        * [ Snowflake prediction job examples ](pred-job-examples-snowflake.html)
      * [ Prediction monitoring jobs ](../pred-monitoring-jobs/index.html) [ Prediction monitoring jobs ](../pred-monitoring-jobs/index.html)
        * [ Create monitoring jobs ](../pred-monitoring-jobs/ui-monitoring-jobs.html)
        * [ Monitoring jobs API ](../pred-monitoring-jobs/api-monitoring-jobs.html)
        * [ Manage monitoring job definitions ](../pred-monitoring-jobs/manage-monitoring-job-def.html)
      * [ Manage batch jobs ](../batch-jobs.html)
      * [ Batch prediction scripts ](../cli-scripts.html)
    * [ Portable prediction methods ](../../port-pred/index.html) [ Portable prediction methods ](../../port-pred/index.html)
      * [ Scoring Code ](../../port-pred/scoring-code/index.html) [ Scoring Code ](../../port-pred/scoring-code/index.html)
        * [ Download Scoring Code from the Leaderboard ](../../port-pred/scoring-code/sc-download-leaderboard.html)
        * [ Download Scoring Code from a deployment ](../../port-pred/scoring-code/sc-download-deployment.html)
        * [ Download Scoring Code from the Leaderboard (Legacy) ](../../port-pred/scoring-code/sc-download-legacy.html)
        * [ Scoring Code for time series projects ](../../port-pred/scoring-code/sc-time-series.html)
        * [ Scoring at the command line ](../../port-pred/scoring-code/scoring-cli.html)
        * [ Scoring Code usage examples ](../../port-pred/scoring-code/quickstart-api.html)
        * [ JAR structure ](../../port-pred/scoring-code/jar-package.html)
        * [ Generate Java models in an existing project ](../../port-pred/scoring-code/build-verify.html)
        * [ Backward-compatible Java API ](../../port-pred/scoring-code/java-back-compat.html)
        * [ Scoring Code JAR integrations ](../../port-pred/scoring-code/sc-jar-integrations.html)
        * [ Android integration ](../../port-pred/scoring-code/android.html)
      * [ Portable Prediction Server ](../../port-pred/pps/index.html) [ Portable Prediction Server ](../../port-pred/pps/index.html)
        * [ Portable Prediction Server configuration ](../../port-pred/pps/portable-pps.html)
        * [ Portable Prediction Server running modes ](../../port-pred/pps/pps-run-modes.html)
        * [ Portable batch predictions ](../../port-pred/pps/portable-batch-predictions.html)
        * [ Custom model Portable Prediction Server ](../../port-pred/pps/custom-pps.html)
      * [ DataRobot Prime (deprecated) ](../../port-pred/prime/index.html)
    * [ Predictions testing ](../../pred-test.html)
    * [ Predictions reference ](../../pred-file-limits.html)
...
[Predictions](../../index.html "Predictions") > [Batch prediction
methods](../index.html "Batch prediction methods") > [Batch prediction
UI](index.html "Batch prediction UI") > Make a one-time batch prediction

# Make a one-time batch prediction¶

Use the **Deployments > Make Predictions** tab to efficiently score datasets
with a deployed model by making batch predictions.

Note

To make predictions with a model before deployment, select the model from the
**Leaderboard** and navigate to [**Predict > Make
Predictions**](../../../modeling/analyze-models/predictions/predict.html).

Batch predictions are a method of making predictions with large datasets, in
which you pass input data and get predictions for each row. DataRobot writes
these predictions to output files. You can also:

  * Schedule [Batch Prediction Jobs](batch-pred-jobs.html) by specifying the prediction data source and destination and determining when DataRobot runs the predictions.

  * Make predictions with the [Batch Prediction API](../../../api/reference/batch-prediction-api/index.html).

## Select a prediction source¶

To make batch predictions with a deployed model, navigate to the deployment's
**Predictions > Make Predictions** tab and upload a prediction source:
...
[![](../../../images/batch-2.png)](../../../images/batch-2.png)

Note

When uploading a prediction dataset, it is automatically stored in the **AI
Catalog** once the upload is complete. Be sure not to navigate away from the
page during the upload, or the dataset will not be stored in the catalog. If
the dataset is still processing after the upload, that means DataRobot is
[running EDA](../../../reference/data-ref/eda-explained.html) on the dataset
before it becomes available for use.

## Make predictions with a deployment¶

This section explains how to use the **Make Predictions** tab to make batch
predictions for standard deployments and time series deployments.

[![](../../../images/make-batch-predictions-ui.png)](../../../images/make-
batch-predictions-ui.png)
...
Making predictions with time series models requires the dataset to be in a
particular format. The format is based on your time series project settings.
Ensure the prediction dataset includes the correct historical rows, forecast
rows, and any features known in advance. In addition, to ensure DataRobot can
process your time series data, configure the dataset to meet the following
requirements:
...
There is _no limit_ on the number of series DataRobot supports. The only limit
is the job timeout, as mentioned in [Limits](../../../api/reference/batch-
prediction-api/index.html#limits). For dataset examples, see the [requirements
for the scoring dataset](../../../api/reference/batch-prediction-api/batch-
pred-ts.html#requirements-for-the-scoring-dataset).

To configure the **Time series options** , under **Time series prediction
method** , define the **Forecast point** settings:
...
[![](../../../images/ts-makepred-1.png)](../../../images/ts-makepred-1.png)

  * **Set manually** : Set a specific date range using the date selector, configuring the **Start** and **End** dates manually.

[![](../../../images/ts-batch-pred-forecast-pt-fixed.png)](../../../images/ts-
batch-pred-forecast-pt-fixed.png)

In addition, you can click **Show advanced options** and enable **Ignore
missing values in known-in-advance columns** to make predictions even if the
provided source dataset is missing values in the known-in-advance columns;
however, this may negatively impact the computed predictions.

## Set prediction options¶

Once the file is uploaded, configure the **Prediction options**. (Optional)
You can click **Show advanced options** to configure additional options.

[![](../../../images/batch-3.png)](../../../images/batch-3.png)
...
2 | Include Prediction Explanations | Adds columns for [Prediction Explanations](../../../modeling/analyze-models/understand/pred-explain/index.html) to your prediction output.
  ...
  * **Low prediction threshold** : Enable and define this threshold to provide Prediction Explanations for _any_ values _below_ the set threshold value.
  * **High prediction threshold** : Enable and define this threshold to provide Prediction Explanations for _any_ values _above_ the set threshold value.
  * **Number of ngram explanations** : Enable and define the maximum number of text [ngram](../../../reference/glossary/index.html#n-gram) explanations to return per row in the dataset. The default (and recommended) setting is **all** (no limit).

_For multiclass models with_ , use the **Classes** settings to control the
method for selecting which classes are used in explanation computation:
...
If you can't enable Prediction Explanations, see Why can't I enable Prediction
Explanations?.  
3 | Include prediction outlier warning | Includes warnings for [outlier prediction values](../../../mlops/deployment-settings/humility-settings.html#prediction-warnings) (only available for regression model deployments).  
4 | Store predictions for data exploration | Tracks [data drift](../../../mlops/monitor/data-drift.html), [accuracy](../../../mlops/monitor/deploy-accuracy.html), [fairness](../../../mlops/governance/mlops-fairness.html), and [data exploration](../../../api/reference/sdk/data-exploration.html) (if enabled for the deployment).  
...
8 | Use default prediction instance | Lets you change the [prediction instance](../../../mlops/deployment/prediction-env/pred-env.html#prediction-environments). Turn the toggle off to select a prediction instance.  
...
The batch prediction process chunks your data into smaller pieces and scores
those pieces one by one, allowing DataRobot to score large batches. The
**Chunk size** setting determines the strategy DataRobot uses to chunk your
data. DataRobot recommends the default setting of **Auto** chunking, as it
performs the best overall; however, other options are available:
...
After you configure predictions settings and click **Compute and download
predictions** to score the data, wait for the prediction job to complete. You
can perform the following actions on completed prediction jobs:

[![](../../../images/batch-4.png)](../../../images/batch-4.png)
...
Predictions are available for download on the **Predictions > Make
Predictions** tab for the next 48 hours. You can also view and download
predictions and logs on the [**Deployments > Batch Jobs**](../batch-jobs.html)
tab.

Cancel a batch prediction job

Click the stop icon  while the job is running to cancel it. For canceled or
failed jobs, click the logs icon  to view the logs for the job.

[![](../../../images/batch-5.png)](../../../images/batch-5.png)

[ DataRobot docs ](../../index.html "DataRobot docs")
  ...
  * [ MLOps ](../index.html) [ MLOps ](../index.html)
    ...
    * [ Deployment settings ](index.html) [ Deployment settings ](index.html)
      * [ Set up service health monitoring ](service-health-settings.html)
      * [ Set up data drift monitoring ](data-drift-settings.html)
      * [ Set up accuracy monitoring ](accuracy-settings.html)
      * [ Set up fairness monitoring ](fairness-settings.html)
      * [ Set up humility rules ](humility-settings.html)
      * [ Configure retraining ](retraining-settings.html)
      * [ Configure challengers ](challengers-settings.html)
      * [ Configure predictions settings ](predictions-settings.html) [ Configure predictions settings ](predictions-settings.html)
...
Configure predictions settings
      ...
      * [ Enable data exploration ](data-exploration-settings.html)
      * [ Set up custom metrics monitoring ](custom-metrics-settings.html)
      * [ Set up timeliness tracking ](usage-settings.html)
...
[MLOps](../index.html "MLOps") > [Deployment settings](index.html "Deployment
settings") > Configure predictions settings

# Configure predictions settings¶

On a deployment's **Predictions > Settings** tab, you can view details about
your deployment's inference (also known as scoring) data—the data containing
prediction requests and results from the model.

On the **Predictions Settings** page, you can access the following
information:

[![](../../images/predictions-settings.png)](../../images/predictions-
settings.png)

Field | Description  
---|---  
Prediction environment | Displays the environment where predictions are generated. [Prediction environments](../deployment/prediction-env/index.html) allow you to establish access controls and approval workflows.  
Prediction timestamp | Defines the method used for time-stamping prediction rows. To define the timestamp, select one of the following:
  ...
  * **Use time of prediction request** : The timestamp of the prediction request.
  * **Use value from date/time feature** : A date/time feature (e.g., forecast date) provided with the prediction data and defined by the **Feature** and **Date format** settings. Forecast date time-stamping is set automatically for time series deployments. It allows for a common time axis to be used between training data and the basis of data drift and accuracy statistics. The **Feature** and **Date format** settings cannot be changed after predictions are made.

  
Batch monitoring | Enables viewing monitoring statistics organized by batch, instead of by time, with [batch-enabled deployments](../monitor/deploy-batch-monitor.html)  
  
## Set prediction autoscaling settings for DataRobot serverless deployments¶

Autoscaling is a system that automatically adjusts the number of model
replicas allocated to your deployment based on the incoming prediction request
load. This ensures your deployment can handle peak traffic without manual
intervention and scales down during quieter periods to optimize resource
usage. For DataRobot models, DataRobot performs autoscaling based on CPU usage
at a 40% threshold.

To configure on-demand predictions on this environment, scroll down to
**Autoscaling** and set the following options:

[![](../../images/nxt-real-time-pred-configure.png)](../../images/nxt-real-
time-pred-configure.png)
...
Always-on predictions are a premium feature. Contact your DataRobot
representative or administrator for information on enabling the feature.
...
For DataRobot model deployments:
  ...
  * The minimum and maximum limits are taken from the organization's `max_compute_serverless_prediction_api` setting.

For custom model deployments:

  * The default minimum is 0 and default maximum is 1.
  * The minimum and maximum limits are taken from the organization's `max_custom_model_replicas_per_deployment` setting.
  * The minimum is always greater than 1 when running on GPUs (for LLMs).

## Set feature discovery project settings¶

Feature discovery projects use multiple datasets to generate new features,
eliminating the need to perform manual feature engineering to consolidate
those datasets into one. For deployed [feature discovery
projects](../../data/transform-data/feature-discovery/fd-overview.html), you
can manage your **Feature discovery** configuration on the **Predictions
Settings** page.
...
[![](../../images/feature-discovery-settings.png)](../../images/feature-
discovery-settings.png)
  ...
  3. If you need to update your secondary datasets, click **Change** to open the **Secondary Datasets Configuration** dialog box, where you can:
     ...
     * Click the menu icon ([![](../../images/icon-menu.png)](../../images/icon-menu.png)) to **Preview** , **Select** , or **Delete** a configuration.

[![](../../images/secondary-datasets-settings.png)](../../images/secondary-
datasets-settings.png)

Note

You can't delete the **Default Configuration** or the **Selected**
configuration.
...
Time series users have the additional capability to add a prediction interval
to the prediction response of deployed models. When enabled, prediction
intervals will be added to the response of any prediction call associated with
the deployment.

To enable prediction intervals, navigate to the **Predictions** > **Prediction
Intervals** tab, click the **Enable prediction intervals** toggle, and select
an **Interval size** (read more about prediction intervals
[here](../../modeling/time/ts-predictions.html#prediction-preview)):

[![](../../images/deploy-7.png)](../../images/deploy-7.png)

After you set an interval, copy the deployment ID from the [Overview
tab](../monitor/dep-overview.html), the deployment URL, or the snippet in the
[**Prediction API**](../../predictions/realtime/code-py.html) tab to check
that the deployment was added to the database. You can compare the results
from your API output with [prediction preview](../../modeling/time/ts-
predictions.html#prediction-preview) in the UI to verify results.

[![](../../images/deploy-8.png)](../../images/deploy-8.png)

For more information on working with prediction intervals via the API, access
the API documentation by signing in to DataRobot, clicking the question mark
on the upper right, and selecting **API Documentation**. In the API
documentation, select **Time Series Projects > Prediction Intervals**.

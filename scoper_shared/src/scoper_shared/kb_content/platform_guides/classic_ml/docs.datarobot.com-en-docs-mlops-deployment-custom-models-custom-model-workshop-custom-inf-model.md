[ DataRobot docs ](../../../../index.html "DataRobot docs")
  ...
  * [ MLOps ](../../../index.html) [ MLOps ](../../../index.html)
    * [ Deployment ](../../index.html) [ Deployment ](../../index.html)
      ...
      * [ Prepare custom models for deployment ](../index.html) [ Prepare custom models for deployment ](../index.html)
        * [ Custom Model Workshop ](index.html) [ Custom Model Workshop ](index.html)
          * [ Create custom inference models ](custom-inf-model.html) [ Create custom inference models ](custom-inf-model.html)
...
Create custom inference models
          ...
          * [ Manage custom model dependencies ](custom-model-dependencies.html)
          * [ Manage custom model resources ](custom-model-resource-mgmt.html)
          * [ Add custom model versions ](custom-model-versions.html)
          * [ Add training data to a custom model ](custom-model-training-data.html)
          * [ Add files from remote repos to custom models ](custom-model-repos.html)
          * [ Test custom models ](custom-model-test.html)
          * [ Manage custom models ](custom-model-actions.html)
          * [ Register custom models ](custom-model-reg.html)
          * [ Create custom model proxies for external models ](ext-model-proxy.html)
          * [ GitHub Actions for custom models ](custom-model-github-action.html)
        * [ Custom model environments ](../custom-model-environments/index.html) [ Custom model environments ](../custom-model-environments/index.html)
          * [ Drop-in environments ](../custom-model-environments/drop-in-environments.html)
          * [ Custom environments ](../custom-model-environments/custom-environments.html)
...
"Deployment") > [Prepare custom models for deployment](../index.html "Prepare
custom models for deployment") > [Custom Model Workshop](index.html "Custom
Model Workshop") > Create custom inference models

# Create custom inference models¶

Custom inference models are user-created, pre-trained models that you can
upload to DataRobot (as a collection of files) via the **Custom Model
Workshop**. You can then upload a model artifact to create, test, and deploy
custom inference models to DataRobot's centralized deployment hub.

You can assemble custom inference models in either of the following ways:

  * Create a custom model _without_ providing the model requirements and `start_server.sh` file on the **Assemble** tab. This type of custom model _must_ use a drop-in environment. Drop-in environments contain the requirements and `start_server.sh` file used by the model. They are [provided by DataRobot](../custom-model-environments/drop-in-environments.html) in the Custom Model Workshop. You can also [create your own](../custom-model-environments/custom-environments.html#create-a-custom-environment) drop-in custom environment.

  * Create a custom model _with_ the model requirements and `start_server.sh` file on the **Assemble** tab. This type of custom model can be paired with a [custom](../custom-model-environments/custom-environments.html#create-a-custom-environment) or [drop-in](../custom-model-environments/drop-in-environments.html) environment.

Be sure to review the guidelines for [assembling a custom
model](../../../../api/code-first-tools/drum/index.html) before proceeding. If
any files overlap between the custom model and the environment folders, the
model's files will take priority.

Note

Once a custom model's file contents are assembled, you can [test the contents
locally](../../../../api/code-first-tools/drum/custom-local-test.html) for
development purposes before uploading it to DataRobot. After you create a
custom model in the Workshop, you can run a [testing suite](custom-model-
test.html) from the **Assemble** tab.
...
[![](../../../../images/cmodel-1.png)](../../../../images/cmodel-1.png)

  2. In the **Add Custom Inference Model** window, enter the fields described in the table below:

[![](../../../../images/cmodel-2.png)](../../../../images/cmodel-2.png)

| Element | Description  
---|---|---  
1 | Model type | Select **Custom model** or [**Proxy**](ext-model-proxy.html) (external).  
2 | Model name | Name the custom model.  
3 | Target type / Target name | Select the target type ([binary classification](../../../../reference/glossary/index.html#classification), [regression](../../../../reference/glossary/index.html#regression), [multiclass](../../../../reference/glossary/index.html#multiclass), [text generation](../../../monitor/generative-model-monitoring.html) (_premium feature_), anomaly detection, or [unstructured](../../../../api/code-first-tools/drum/unstructured-custom-models.html)) and enter the name of the target feature.  
4 | Positive class label / Negative class label | _For binary classification models_ , specify the value to be used as the positive class label and the value to be used as the negative class label.   
For a multiclass classification model, these fields are replaced by a field to
enter or upload the target classes in `.csv` or `.txt` format.  
  
Target type support for proxy models

If you select the **Proxy** model type, you can't select the **Unstructured**
target type.
...
[![](../../../../images/cmodel-3.png)](../../../../images/cmodel-3.png)

  4. After completing the fields, click **Add Custom Model**.

  5. In the **Assemble** tab, under **Model Environment** on the right, select a model environment by clicking the **Base Environment** dropdown menu on the right and selecting an environment. The model environment is used for [testing](custom-model-test.html) and [deploying](../../deploy-methods/deploy-custom-inf-model.html) the custom model.

[![](../../../../images/cmodel-assemble-add-
env.png)](../../../../images/cmodel-assemble-add-env.png)

Note

The **Base Environment** pulldown menu includes [drop-in model
environments](../custom-model-environments/drop-in-environments.html), if any
exist, as well as [custom environments](../custom-model-environments/custom-
environments.html#create-a-custom-environment) that you can create.

  6. Under **Model** on the left, add content by dragging and dropping files or browsing. Alternatively, select a [remote integrated repository](custom-model-repos.html).

[![](../../../../images/cmodel-assemble-add-
files.png)](../../../../images/cmodel-assemble-add-files.png)

If you click **Browse local file** , you have the option of adding a **Local
Folder**. The local folder is for dependent files and additional assets
required by your model, not the model itself. Even if the model file is
included in the folder, it will not be accessible to DataRobot unless the file
exists at the root level. The root file can then point to the dependencies in
the folder.

Note

You must also upload the model requirements and a `start_server.sh` file to
your model's folder unless you are pairing the model with a [drop-in
environment](../custom-model-environments/drop-in-environments.html).

### View and edit runtime parameters¶

When you add a `model-metadata.yaml` file with `runtimeParameterDefinitions`
to DataRobot while creating a custom model, the **Runtime Parameters** section
appears on the **Assemble** tab for that custom model. After you build the
environment and create a new version, you can click **View and Edit** to
configure the parameters:

[![](../../../../images/assemble-tab-runtime-
params.png)](../../../../images/assemble-tab-runtime-params.png)

Note

Each change to a runtime parameter creates a new minor version of the custom
model.

After you [test a model](custom-model-test.html) with runtime parameters in
the **Custom Model Workshop** , you can navigate to the **Test > Runtime
Parameters** to view the model's parameters:

[![](../../../../images/test-tab-runtime-params.png)](../../../../images/test-
tab-runtime-params.png)

If any runtime parameters have `allowEmpty: false` in the definition without a
`defaultValue`, you must set a value before registering the custom model.

For more information on how to define runtime parameters and use them in your
custom model code, see the [Define custom mode runtime
parameters](../../../../api/code-first-tools/drum/custom-model-runtime-
parameters.html) documentation.

### Anomaly detection¶

You can create custom inference models that support anomaly detection
problems. If you choose to build one, reference the [DRUM
template](https://github.com/datarobot/datarobot-user-
models/tree/master/model_templates/python3_sklearn_anomaly). (Log in to GitHub
before clicking this link.) When deploying custom inference anomaly detection
models, note that the following functionality is not supported:

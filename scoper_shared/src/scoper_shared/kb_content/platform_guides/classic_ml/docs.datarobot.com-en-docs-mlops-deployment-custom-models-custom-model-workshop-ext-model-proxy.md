    ...
    * [ Deployment ](../../index.html) [ Deployment ](../../index.html)
      ...
      * [ Prepare custom models for deployment ](../index.html) [ Prepare custom models for deployment ](../index.html)
        * [ Custom Model Workshop ](index.html) [ Custom Model Workshop ](index.html)
          * [ Create custom inference models ](custom-inf-model.html)
          ...
          * [ Create custom model proxies for external models ](ext-model-proxy.html) [ Create custom model proxies for external models ](ext-model-proxy.html)
...
Model Workshop") > Create custom model proxies for external models

# Create custom model proxies for external models¶

To create a custom model as a proxy for an external model, you can add a new
proxy model to the [Custom Model Workshop](index.html). A proxy model contains
proxy code created to connect with an external model, allowing you to use
features like [compliance documentation](../../registry/reg-compliance.html),
[challenger analysis](../../../monitor/challengers.html), and [custom model
tests](custom-model-test.html) with a model running on infrastructure outside
DataRobot.

To create a proxy model:

  1. (Optional) [Add runtime parameters](../../../../api/code-first-tools/drum/custom-model-runtime-parameters.html) to the custom model through the model metadata (`model-metadata.yaml`).
...
The custom model you create as a proxy for an external model should contain
custom code in the `custom.py` file to connect the proxy model with the
externally-hosted model; this code is the _proxy code_. See the [custom model
assembly](../../../../api/code-first-tools/drum/index.html) documentation for
more information on writing custom model code.

The proxy code in the `custom.py` file should do the following:

  * Import the necessary modules and, optionally, the [runtime parameters](../../../../api/code-first-tools/drum/custom-model-runtime-parameters.html) from `model-metadata.yaml`.
...
To simplify the reuse of proxy code, you can add [runtime
parameters](../../../../api/code-first-tools/drum/custom-model-runtime-
parameters.html) through your model metadata in the `model-metadata.yaml`
file:
...
If you define runtime parameters in the model metadata, you can import them
into the `custom.py` file to use in your proxy code. After importing these
parameters, you can assign them to variables in your proxy code. This allows
you to create a prediction request to connect to and retrieve prediction data
from the external model. The following example outlines the basic structure of
a `custom.py` file:
...
To create a custom model as a proxy for an external model, you can add a new
proxy model to the [Custom Model Workshop](index.html). A proxy model contains
the proxy code you created to connect with your external model, allowing you
to use features like [compliance documentation](../../registry/reg-
compliance.html), [challenger analysis](../../../monitor/challengers.html),
and [custom model tests](custom-model-test.html) with a model running on
infrastructure outside DataRobot.
...
[![](../../../../images/add-proxy-model.png)](../../../../images/add-proxy-
model.png)

Field | Description  
---|---  
Model name | Name the custom model.  
Target type / Target name | Select the target type ([binary classification](../../../../reference/glossary/index.html#classifiction), [regression](../../../../reference/glossary/index.html#regression), [multiclass classification](../../../../reference/glossary/index.html#multiclass), [anomaly detection](custom-inf-model.html#anomaly-detection), or [text-generation](../../../monitor/generative-model-monitoring.html) (_premium feature_)) and enter the name of the target feature.  
...
environments.html#create-a-custom-environment) that you can create.
  ...
  7. Under **Model** on the left, add proxy model content by dragging and dropping files or browsing. Alternatively, select a [remote integrated repository](custom-model-repos.html).
...
environment](../custom-model-environments/drop-in-environments.html).
  ...
  8. On the **Assemble** tab, next to **Resource settings** , click the edit icon ([![](../../../../images/icon-pencil.png)](../../../../images/icon-pencil.png)) to activate the required **Network access** for the proxy model.
  ...
  10. Finally, you can [register the custom model](custom-model-reg.html) to create a proxy model you can use to generate [compliance documentation](../../registry/reg-compliance.html). You can then deploy the proxy model to set up [challenger analysis](../../../monitor/challengers.html) and run [custom model tests](custom-model-test.html) on the external model.

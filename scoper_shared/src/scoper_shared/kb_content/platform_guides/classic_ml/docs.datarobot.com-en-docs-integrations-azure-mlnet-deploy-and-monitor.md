[ DataRobot docs ](../../index.html "DataRobot docs")
  ...
  * [ Integrations ](../index.html) [ Integrations ](../index.html)
    ...
    * [ Azure ](index.html) [ Azure ](index.html)
      ...
      * [ Deploy and monitor Spark models with DataRobot MLOps ](spark-deploy-and-monitor.html)
      * [ Deploy and monitor ML.NET models with DataRobot MLOps ](mlnet-deploy-and-monitor.html) [ Deploy and monitor ML.NET models with DataRobot MLOps ](mlnet-deploy-and-monitor.html)
...
This page explores how models built with ML.NET can be deployed and monitored
with DataRobot MLOps. ML.NET is an open-source machine learning framework
created by Microsoft for the .NET developer platform. To learn more, see [What
is ML.NET?](https://dotnet.microsoft.com/learn/ml-dotnet/what-is-mldotnet).

The examples on this page use the [public LendingClub
dataset](https://datarobot-doc-assets.s3.us-
east-1.amazonaws.com/10K_Lending_Club_Loans.csv).

You want to predict the likelihood of a loan applicant defaulting; in machine
learning, this is referred to as a "binary classification problem." You can
solve this with DataRobot AutoML, but in this case, you will create the model
with ML.NET, deploy it into production, and monitor it with DataRobot MLOps.
DataRobot MLOps allows you to monitor all of your models in one central
dashboard, regardless of the source or programming language.

Before deploying a model to DataRobot MLOps, you must create a new ML.NET
model, and then create an ML.NET environment for DataRobot MLOps.

Note

This DataRobot MLOps ML.NET environment only has to be created once, and if
you only require support for binary classification and regression models, you
can skip this step and use the existing "DataRobot ML.NET Drop-In" environment
from the [DataRobot Community GitHub](https://github.com/datarobot-
community/ai_engineering/tree/master/mlops/DRMLOps_MLNET_environment).

## Prerequisites¶

To start building .NET apps, download and install the .NET software
development kit (SDK). To install the SDK, follow the steps outlined in the
[.NET intro](https://dotnet.microsoft.com/learn/dotnet/hello-world-
tutorial/intro).
...
If the installation of the ML.NET framework is successful, create the ML.NET
model:
...
After the ML.NET CLI selects the best model, it displays the experiment
results—a summary of the exploration process—including how many models were
explored in the given training time.

[![](../../images/int_htramnmwdm_large_005.png)](../../images/int_htramnmwdm_large_005.png)

While the ML.NET CLI generates code for the highest performing model, it also
displays up to five models with the highest accuracy found during the given
exploration time. It displays several evaluation metrics for those top models,
including AUC, AUPRC, and F1-score.

### Test your model¶

The ML.NET command-line interface (CLI) generates the machine learning model
and adds the .NET apps and libraries needed to train and consume the model.
The files created include the following:
...
To test the model, run the console app
(`SampleBinaryClassification.ConsoleApp`), which predicts the likelihood of
default for a single applicant:
...
While DataRobot provides many environment templates out of the box (including
R, Python, Java, PyTorch, etc.), this section shows how to create your own
runtime environment, from start to finish, using ML.NET.
...
To make an easy-to-use, reusable environment, follow the below guidelines:
  ...
  * A Dockerfile that copies all code and the `start_server.sh` script to `/opt/code/`. You can download the code for [the MLOps environment package](https://github.com/datarobot-community/ai_engineering/tree/master/mlops/DRMLOps_MLNET_model%20samples) from the DataRobot Community GitHub.
...
---|---  
[![](../../images/mldotnet-request-1.png)](../../images/mldotnet-request-1.png) | [![](../../images/mldotnet-request-1.png)](../../images/mldotnet-request-1.png)  
...
DataRobot MLOps runs extensive tests before deploying a custom model to ensure
reliability; therefore, it is essential that your webserver handles missing
values and returns results in the expected response format as outlined above.

As previously mentioned, you need to use port `8080` so that DataRobot can
correctly identify the webserver. Therefore, in `appsettings.json`, specify
port `8080` for the `Kestrel` web server, as shown below:
...
Initialize the model code (`mlContext`, `mlModel`, and `predEngine`) in the
`Startup.cs` class. This allows .NET to recognize file changes whenever you
...
The `start_server.sh` shell script is responsible for starting the model
server in the container. If you packaged the model and server together, you
only need the compiled version, and the shell script runs `dotnet
consumeModelApp.dll`. Since you have the model code and the server environment
code separated for reusability, recompile from the source at container
startup, as in the command below:
...
Before uploading your custom environment to DataRobot MLOps, compress your
custom environment code to a tarball, as shown below:
...
To upload the new MLOps ML.NET environment, refer to the instructions on
[creating a new custom model environment](../../mlops/deployment/custom-
models/custom-model-environments/custom-environments.html) (see the screenshot
below).

[![](../../images/int_htramnmwdm_large_006.png)](../../images/int_htramnmwdm_large_006.png)

## Upload and test the ML.NET model in DataRobot MLOps¶

Once the environment is created, create a new custom model entity and upload
the actual model (`MLModel.zip` and `ModelInput.cs`).

To upload the new ML.NET model, refer to the instructions on [creating a new
custom inference model](../../mlops/deployment/custom-models/custom-model-
workshop/custom-inf-model.html).

[![](../../images/int_htramnmwdm_large.jpg)](../../images/int_htramnmwdm_large.jpg)

After creating the environment and model within DataRobot MLOps, upload test
data to confirm it works as expected (as shown in the following screenshot).

[![](../../images/int_htramnmwdm_large.png)](../../images/int_htramnmwdm_large.png)

During this phase, DataRobot runs a test to determine how the model handles
missing values and whether or not the internal webserver adheres to the
response format.
...
[![](../../images/int_htramnmwdm_large_002.png)](../../images/int_htramnmwdm_large_002.png)

When this step is complete, you can make predictions with your new custom
model as with any other DataRobot model (see also [Postman
collection](https://github.com/datarobot-
community/ai_engineering/blob/master/mlops/DRMLOps_MLNET_model%20samples/DR%20MLNET%20Custom%20Models.postman_collection.json)):

[![](../../images/int_htramnmwdm_large_003.png)](../../images/int_htramnmwdm_large_003.png)

Even though you built the model outside of DataRobot with ML.NET, you can use
it like any other DataRobot model and track service health and data drift in
one central dashboard, shown below:

  ...
  * [ Modeling ](../../../index.html) [ Modeling ](../../../index.html)
    ...
    * [ Model insights ](../../index.html) [ Model insights ](../../index.html)
      ...
      * [ Understand ](../index.html) [ Understand ](../index.html)
        ...
        * [ Feature Effects ](../feature-effects.html)
        * [ Feature Impact ](../feature-impact.html)
        * [ Prediction Explanations ](index.html) [ Prediction Explanations ](index.html)
          * [ Prediction Explanations overview ](predex-overview.html) [ Prediction Explanations overview ](predex-overview.html)
...
Prediction Explanations overview
          ...
          * [ SHAP Prediction Explanations ](shap-pe.html)
          * [ XEMP Prediction Explanations ](xemp-pe.html)
          * [ Text Prediction Explanations ](predex-text.html)
          * [ Prediction Explanations for clusters ](cluster-pe.html)
          * [ Prediction Explanations for time-aware projects ](ts-otv-predex.html)
...
"Model insights") > [Understand](../index.html "Understand") > [Prediction
Explanations](index.html "Prediction Explanations") > Prediction Explanations
overview

# Prediction Explanations overview¶

**Prediction Explanations** illustrate what drives predictions on a row-by-row
basis—they provide a quantitative indicator of the effect variables have on
the predictions, answering why a given model made a certain prediction. It
helps to understand why a model made a particular prediction so that you can
then validate whether the prediction makes sense. It's especially important in
cases where a human operator needs to evaluate a model decision and also when
a model builder needs to confirm that the model works as expected. For
example, "why does the model give a 94.2% chance of readmittance?" (See more
examples below.)

DataRobot offers two methodologies for computing Prediction Explanations: SHAP
(based on Shapley Values) and XEMP (eXemplar-based Explanations of Model
Predictions).

Enable SHAP in DataRobot Classic

In the DataRobot Classic UI, to avoid confusion when the same insight is
produced yet potentially returns different results, you must enable SHAP in
[**Advanced options**](../../../build-models/adv-opt/additional.html) prior to
project start.

DataRobot also provides [Text Prediction Explanations](predex-text.html)
specific to text features, which help to understand, at the word level, the
text and its influence on the model. Text Prediction Explanations support both
XEMP and SHAP methodologies.

To access and enable Prediction Explanations, select a model on the
Leaderboard and click **Understand > Prediction Explanations**.

[![](../../../../images/leader-pred-explain.png)](../../../../images/leader-
pred-explain.png)
...
Both SHAP and XEMP methodologies estimate which features have stronger or
weaker impact on the target for a particular row. While the two methodologies
usually provide similar results, the explanation _values_ are different
(because methodologies are different). The list below illustrates some
differences:
...
Model support | _NextGen_ : All models  
  
 _DataRobot Classic_ : [Linear models, Keras deep learning models, and tree-based models, including tree ensembles](../../../../reference/pred-ai-ref/shap-ref.html#classic-only-shap-compatibility-matrix). | XEMP works for all models.  
...
While Prediction Explanations provide several quantitative indicators for why
a prediction was made, the calculations do not fully explain _how_ a
prediction is computed. For that information, use the [coefficients with
preprocessing
information](../../describe/coefficients.html#coefficientpreprocessing-
information-with-text-variables) from the **Coefficients** tab.

See the [XEMP](xemp-pe.html) or [SHAP](shap-pe.html) pages for a methodology-
based description of using and interpreting Prediction Explanations.

## Examples¶

A common question when evaluating data is “why is a certain data point
considered high-risk (or low-risk) for a certain event”?

A sample case for **Prediction Explanations** :

> Sam is a business analyst at a large manufacturing firm. She does not have a
> lot of data science expertise, but has been using DataRobot with great
> success to predict the likelihood of product failures at her manufacturing
> plant. Her manager is now asking for recommendations for reducing the defect
> rate, based on these predictions. Sam would like DataRobot to produce
> Prediction Explanations for the expected product failures so that she can
> identify the key drivers of product failures based on a higher-level
> aggregation of explanations. Her business team can then use this report to
...
_SHAP_ estimates how much a feature is responsible for a given prediction
being different from the average. Consider a credit risk example that builds a
simple model with two features—number of credit cards and employment status.
The model predicts that an unemployed applicant with 10 credit cards has a 50%
probability of default, while the average default rate is 5%. SHAP estimates
how each feature contributed to the 50% default risk prediction, determining
that 25% is attributed to the number of cards and 20% is due to the customer's
lack of a job.

## Feature considerations¶

Consider the following when using Prediction Explanations. See also [time-
series specific](../../../../reference/pred-ai-ref/ts-reference/ts-
consider.html#accuracy) considerations.
  ...
  * [Image Explanations](xemp-pe.html#prediction-explanations-for-visual-ai)—or Prediction Explanations for images—are not available from a deployment (for example, Batch predictions or the Predictions API). See also SHAP considerations below.
  ...
  * The deployment [Data Exploration](../../../../api/reference/sdk/data-exploration.html) tab doesn't store the Prediction Explanations for export, even when Prediction Explanations are requested while making predictions through that deployment.

### Prediction Explanation and Feature Impact methods¶

Prediction Explanations and Feature Impact are calculated in multiple ways
depending on the project and target type:

Note

SHAP Impact is an aggregation of SHAP explanations. For more information, see
[SHAP-based Feature Impact](../feature-impact.html#shap-based-feature-impact)

Non-time series / Out-of-time validationTime series

Target type | Feature Impact method | Prediction Explanations method  
---|---|---  
Regression | Permutation Impact or SHAP Impact | XEMP or [SHAP (opt-in when using the Classic UI)](../../../build-models/adv-opt/additional.html)  
Binary | Permutation Impact or SHAP Impact | XEMP or [SHAP (opt-in when using the Classic UI)](../../../build-models/adv-opt/additional.html)  
...
_** For the time series unsupervised anomaly detection visualizations,
the[Anomaly Assessment chart](../../evaluate/anom-viz.html#anomaly-assessment)
uses SHAP to calculate explanations for anomalous points._

### XEMP¶

Consider the following when using XEMP (which is based on permutation-based
...
### SHAP¶
  ...
  * For AutoML, SHAP is only supported by linear, tree-based, and Keras deep learning blueprints. Most of the non-blender AutoML BPs that typically appear at the top of Leaderboard are supported (see the [compatibility matrix](../../../../reference/pred-ai-ref/shap-ref.html#classic-only-shap-compatibility-matrix).
  ...
  * SHAP is not supported for:
    ...
    * Blender models
  * SHAP does not support image feature types. As a result, [Image Explanations](xemp-pe.html#prediction-explanations-for-visual-ai) are not available.
  ...
  * Limits on the number of explanations available:
    ...
    * UI: Prediction Explanations preview is limited to five explanations; however, you can [download](shap-pe.html#compute-and-download) up to 100 via CSV or use the API if you need access to more than 100 explanations. Feature Impact preview is limited to 25 features, but you can export up to 1000 to CSV.

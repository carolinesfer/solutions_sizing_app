...
from scoper_shared.schemas import QuestionnaireDraft
...
def chat(
    ...
    **kwargs: Any,
    ...
    a JSON string with QuestionnaireDraft data.
    ...
    Example:
        ...
        completion = client.chat.completions.create(
            messages=[
                ...
                {"role": "user", "content": '{"questions": [...], "selected_from_master_ids": [...], ...}'},
            ],
            ...
        )
    """
    ...
    # Parse user prompt as QuestionnaireDraft JSON
    try:
        if user_prompt.strip().startswith("{"):
            draft = QuestionnaireDraft.model_validate_json(user_prompt)
        else:
            raise ValueError("User prompt must be a JSON string containing QuestionnaireDraft")
    except Exception as e:
        raise ValueError(f"Invalid QuestionnaireDraft JSON: {e}") from e

    # Get model name from params or use default
    model_name = completion_create_params.get("model")

    # Instantiate the agent
    agent = ClarifierAgent(model_name=model_name)

    # Run the agent
    def run_agent() -> tuple[str, list, dict[str, int]]:
        """Run agent in event loop and return formatted result."""
        result = event_loop.run_until_complete(agent.run(draft))

        # Convert QuestionnaireFinal to JSON string for response

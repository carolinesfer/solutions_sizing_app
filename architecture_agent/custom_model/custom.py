...
from scoper_shared.schemas import QuestionnaireFinal
...
def chat(
    ...
    **kwargs: Any,
    ...
    a JSON string with QuestionnaireFinal data.
    ...
    Example:
        ...
        completion = client.chat.completions.create(
            messages=[
                ...
                {"role": "user", "content": '{"qas": [...], "answered_pct": 0.9, ...}'},
            ],
            ...
        )
    """
    ...
    # Parse user prompt as QuestionnaireFinal JSON
    try:
        if user_prompt.strip().startswith("{"):
            questionnaire = QuestionnaireFinal.model_validate_json(user_prompt)
        else:
            raise ValueError("User prompt must be a JSON string containing QuestionnaireFinal")
    except Exception as e:
        raise ValueError(f"Invalid QuestionnaireFinal JSON: {e}") from e

    # Get model name from params or use default
    model_name = completion_create_params.get("model")

    # Instantiate the agent
    agent = ArchitectureAgent(model_name=model_name)

    # Run the agent
    def run_agent() -> tuple[str, list, dict[str, int]]:
        """Run agent in event loop and return formatted result."""
        plan, markdown = event_loop.run_until_complete(agent.run(questionnaire))

        # Combine plan JSON and markdown in response
        # Return both as JSON with markdown field
        response_dict = {
